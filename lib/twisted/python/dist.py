...
Don't use this outside of lib.twisted.
	...
	if 'twisted_subproject' in kw:
		...
		if 'plugins' in kw:
			...
			for plg in kw['plugins']:
				py_modules.append("lib.twisted.plugins." + plg)
...
def getDataFiles(dname, ignore=None, parent=None):
	...
	the 'twisted' package doesn't want to include the 'lib.twisted.conch'
	package, so it will pass ['conch'] as the value.

	'parent' is necessary if you're distributing a subpackage like
	lib.twisted.conch.  'dname' should point to 'twisted/conch' and 'parent'
