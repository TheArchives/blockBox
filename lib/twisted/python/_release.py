# -*- test-case-name: lib.twisted.python.test.test_release -*-
...
class Project(object):
	...
	@ivar directory: A L{lib.twisted.python.filepath.FilePath} pointing to the base
...
def findTwistedProjects(baseDirectory):
	...
	@param baseDirectory: A L{lib.twisted.python.filepath.FilePath} to look inside.
...
def updateTwistedVersionInformation(baseDirectory, now):
	...
	@param baseDirectory: Where to look for lib.twisted. If None, the function
		infers the information from C{lib.twisted.__file__}.
...
class DocBuilder(LoreBuilderMixin):
	...
	def build(self, version, resourceDir, docDir, template, apiBaseURL=None,
		...
		@type resourceDir: L{lib.twisted.python.filepath.FilePath}

		@param docDir: The directory of the documentation.
		@type docDir: L{lib.twisted.python.filepath.FilePath}

		@param template: The template used to generate the documentation.
		@type template: L{lib.twisted.python.filepath.FilePath}
...
class ManBuilder(LoreBuilderMixin):
	...
	def build(self, manDir):
		...
		@type manDir: L{lib.twisted.python.filepath.FilePath}
...
def filePathDelta(origin, destination):
	...
	say, the delta of L{lib.twisted.python.filepath.FilePath} /foo/bar to
	L{lib.twisted.python.filepath.FilePath} /foo/baz will be C{../baz},
	not C{baz}.

	@type origin: L{lib.twisted.python.filepath.FilePath}
	@param origin: The origin of the relative path.

	@type destination: L{lib.twisted.python.filepath.FilePath}
