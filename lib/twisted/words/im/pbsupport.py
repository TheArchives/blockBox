...
"""L{lib.twisted.words} support for Instance Messenger."""
...
from lib.twisted.words.im.locals import ONLINE, OFFLINE, AWAY

from lib.twisted.words.im import basesupport, interfaces
from lib.zope.interface import implements


class TwistedWordsPerson(basesupport.AbstractPerson):
	"""I a facade for a person you can talk to through a lib.twisted.words service.
	"""
...
class TwistedWordsGroup(basesupport.AbstractGroup):
	...
	def sendGroupMessage(self, text, metadata=None):
		"""Return a deferred.
		"""
		#for backwards compatibility with older lib.twisted.words servers.
		if metadata:
...
pbFrontEnds = {
	"lib.twisted.words": TwistedWordsClient,
	"lib.twisted.reality": None
	}
...
class PBAccount(basesupport.AbstractAccount):
	...
	def __init__(self, accountName, autoLogin, username, password, host, port,
		...
		self.services = []
		if not services:
			services = [('lib.twisted.words', 'lib.twisted.words', username)]
