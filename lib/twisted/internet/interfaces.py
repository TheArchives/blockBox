...
class IAddress(Interface):
	...
	Default implementations are in L{lib.twisted.internet.address}.
	"""
...
class IResolverSimple(Interface):
	...
	def getHostByName(name, timeout = (1, 3, 11, 45)):
		...
		@raise lib.twisted.internet.defer.TimeoutError: Raised (asynchronously)
...
class IReactorTCP(Interface):
	...
	def listenTCP(port, factory, backlog=50, interface=''):
		...
		@param factory: a L{lib.twisted.internet.protocol.ServerFactory} instance
		...
		@raise CannotListenError: as defined here
								  L{lib.twisted.internet.error.CannotListenError},
	...
	def connectTCP(host, port, factory, timeout=30, bindAddress=None):
		...
		@param factory: a L{lib.twisted.internet.protocol.ClientFactory} instance
		...
		@return: An object which provides L{IConnector}. This connector will
				 ...
				 L{ClientFactory<lib.twisted.internet.protocol.ClientFactory>}
				 docs for details.
		"""
...
class IReactorSSL(Interface):
	...
	def connectSSL(host, port, factory, contextFactory, timeout=30, bindAddress=None):
		...
		@param contextFactory: a L{lib.twisted.internet.ssl.ClientContextFactory} object.
	...
	def listenSSL(port, factory, contextFactory, backlog=50, interface=''):
		...
		@param contextFactory: a L{lib.twisted.internet.ssl.ContextFactory} instance
...
class IReactorUNIX(Interface):
	...
	def listenUNIX(address, factory, backlog=50, mode=0666, wantPID=0):
		...
		@param factory: a L{lib.twisted.internet.protocol.Factory} instance.
...
class IReactorUNIXDatagram(Interface):
	...
	def connectUNIXDatagram(address, protocol, maxPacketSize=8192, mode=0666, bindAddress=None):
		...
		@param protocol: a L{lib.twisted.internet.protocol.ConnectedDatagramProtocol} instance
	...
	def listenUNIXDatagram(address, protocol, maxPacketSize=8192, mode=0666):
		...
		@param protocol: a L{lib.twisted.internet.protocol.DatagramProtocol} instance.
...
class IReactorWin32Events(Interface):
	...
	def addEvent(event, fd, action):
		...
		@param fd: an instance of L{lib.twisted.internet.abstract.FileDescriptor}
...
class IReactorMulticast(Interface):
	...
	def listenMulticast(port, protocol, interface='', maxPacketSize=8192,
		...
		"""
		Connects a given
		L{DatagramProtocol<lib.twisted.internet.protocol.DatagramProtocol>} to the
		...
		@see: L{lib.twisted.internet.interfaces.IMulticastTransport}
...
class IReactorProcess(Interface):
	...
	def spawnProcess(processProtocol, executable, args=(), env={}, path=None,
		...
		@see: L{lib.twisted.internet.protocol.ProcessProtocol}
...
class IDelayedCall(Interface):
	...
	def cancel():
		...
		@raises lib.twisted.internet.error.AlreadyCalled: if the call has already
			happened.
		@raises lib.twisted.internet.error.AlreadyCancelled: if the call has already
			been cancelled.
		"""
...
class IReactorThreads(Interface):
	...
	def getThreadPool():
		...
		@rtype: L{lib.twisted.python.threadpool.ThreadPool}
		"""
...
class IProtocol(Interface):
	...
	def connectionLost(reason):
		...
		Failure wraps a L{lib.twisted.internet.error.ConnectionDone} or
		L{lib.twisted.internet.error.ConnectionLost} instance (or a subclass
		of one of those).

		@type reason: L{lib.twisted.python.failure.Failure}
		"""
...
class IProcessProtocol(Interface):
	...
	def processExited(reason):
		...
		@param reason: A failure giving the reason the child process
			...
			L{lib.twisted.internet.error.ProcessDone} or
			L{lib.twisted.internet.error.ProcessTerminated}.
...
class ITLSTransport(ITCPTransport):
	...
	def startTLS(contextFactory):
		...
		@param contextFactory: A context factory (see L{ssl.py<lib.twisted.internet.ssl>})
		"""
...
class IProcessTransport(ITransport):
	...
	def signalProcess(signalID):
		...
		@raise lib.twisted.internet.error.ProcessExitedAlready: The process has
		already exited.
		"""
...
class IUDPTransport(Interface):
	...
	def write(packet, addr=None):
		...
		@raise lib.twisted.internet.error.MessageLengthError: C{packet} was too
		long.
		"""
