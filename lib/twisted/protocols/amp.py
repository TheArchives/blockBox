...
class _NoCertificate:
	...
	def options(self, *authorities):
		"""
		Behaves like L{lib.twisted.internet.ssl.PrivateCertificate.options}().
		"""
		if not self.client:
...
class StartTLS(Command):
	...
	TLS negotiation:
		...
		lib.twisted.internet.ssl.PrivateCertificate which will be used to secure
		the side of the connection it is returned on.

		- tls_verifyAuthorities: This is a list of
		lib.twisted.internet.ssl.Certificate objects that will be used as the
...
class BinaryBoxProtocol(StatefulStringProtocol, Int16StringReceiver):
	...
	def _startTLS(self, certificate, verifyAuthorities):
		...
		@param certificate: a L{lib.twisted.internet.ssl.PrivateCertificate} for
		use locally.

		@param verifyAuthorities: L{lib.twisted.internet.ssl.Certificate} instances
