...
"""
Tests for L{lib.twisted.protocols.tls}.
"""

try:
    from lib.twisted.protocols.tls import TLSMemoryBIOProtocol, TLSMemoryBIOFactory
except ImportError:
    # Skip the whole test module if it can't be imported.
    skip = "pyOpenSSL 0.10 or newer required for lib.twisted.protocol.tls"
else:
    ...
    from lib.twisted.internet.ssl import ClientContextFactory, PrivateCertificate
    from lib.twisted.internet.ssl import DefaultOpenSSLContextFactory

from lib.twisted.python.filepath import FilePath
from lib.twisted.internet.interfaces import ISystemHandle, ISSLTransport
from lib.twisted.internet.error import ConnectionDone
from lib.twisted.internet.defer import Deferred, gatherResults
from lib.twisted.internet.protocol import Protocol, ClientFactory, ServerFactory
from lib.twisted.protocols.loopback import loopbackAsync, collapsingPumpPolicy
from lib.twisted.trial.unittest import TestCase
from lib.twisted.test.test_tcp import ConnectionLostNotifyingProtocol
from lib.twisted.test.test_ssl import certPath
from lib.twisted.test.proto_helpers import StringTransport
...
class TLSMemoryBIOTests(TestCase):
    ...
    def test_getHandle(self):
        ...
        knows about directly.  However, L{lib.twisted.internet.ssl.Certificate}'s
