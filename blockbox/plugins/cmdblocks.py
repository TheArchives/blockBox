...
class CommandPlugin(ProtocolPlugin):
	...
	commands = {
		...
		"cmddelprev": "commandDelPreviousCommand",
	...
	@writer_only
	def commanDelPreviousCommand(self, parts, byuser, permissionoverride):
	...
	def runcommands(self):
		...
		if runcmd is True:
			...
			try:
				...
				self.client.sendSplitServerMessage(traceback.format_exc().replace("Traceback (most recent call last):", ""))
				self.client.sendSplitServerMessage("Internal Server Error - Traceback (Please report this to the Server Staff or the blockBox Team, see /about for contact info)")
				self.client.logger(traceback.format_exc())
		self.runningcmdlist.remove(self.runningcmdlist[0])
